...
def lookup(features: Sequence[dict]) -> Tuple[Sequence[dict], Sequence[dict]]:
    ...
    try:
        ...
        for (feature, future) in rec_futures:
            ...
            if(rec is not None):
                hit_times.append(elapsed)   # record hit time
                ...
                miss_times.append(elapsed)   # record miss time
                features_not_found.append(feature)
        ...
        log.warning('IPS hits:   %s', fmt(hit_times))
        log.warning('IPS misses: %s', fmt(miss_times))
