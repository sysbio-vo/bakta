...
def predict_nc_rna_regions(data: dict, sequences_path: Path):
    ...
    if cfg.db_ncrna_regions is not None:
        ncrna_regions_path = cfg.db_ncrna_regions
    elif cfg.db_path is not None:
        ncrna_regions_path = cfg.db_path.joinpath('ncRNA-regions')
    else:
        log.error(f"No ncRNA-regions database path was provided, ncRNA-regions will NOT be detected")
        raise ValueError(f"No ncRNA-regions database path was provided, ncRNA-regions will NOT be detected")
    
    log.info(f"{ncrna_regions_path=}")
    ...
    cmd.append(str(ncrna_regions_path))
